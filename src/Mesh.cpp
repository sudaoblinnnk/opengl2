...
Mesh::Mesh(Vertex *vertices, unsigned int numVertex) {
    ...
    glBufferData(GL_ARRAY_BUFFER, mDrawCount * sizeof(struct Vertex), vertices,
    ...
    glBindBuffer(GL_ARRAY_BUFFER,
                 0);//unbind mVertexArrayBuffers[POSITION_VB], bind buffer call is simply cleanup. By binding the buffer object 0 to GL_ARRAY_BUFFER, we cause the buffer object previously bound to that target to become unbound from it
